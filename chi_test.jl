...
# hypothesis testing for a distribution over categories

💡 topic: a simulation-based approach for testing a hypothesis that an observed distribution over categories differs from an expected distribution. 

!!! note \"Reference\"
	I learned this from the excellent textbook \"Understanding Data\" by Alan Garfinkel and Yina Guo.
...
md"
under the null hypothesis, we can simulate outcomes of customers purchasing drinks, if we assume each customer chooses a drink independently of the others.
"
...
md"
## data
on our first day, 30 customers visited our bar. each purchased a drink. here is the empirical distribution over drinks."
...
χ_obs = sum(abs(data[drink] - Int(n_customers / length(drinks))) for drink in drinks)
...
χs_null = [
	begin
		local sim_data = simulate_null(n_customers, p_null)
		χ = sum(
			abs(sim_data[drink] - Int(n_customers / length(drinks)))
			for drink in drinks
		)
	end
	for s = 1:n_sims
...
md"since the p-value is quite big, we do not reject the null hypothesis.

this doesn't mean the null hypothesis is true. if there really were underlying preference differences among the drinks, a more sizable data set (i.e. more customers) would give us the power to reject the null hypothesis. this is quite a small data set.
"

# ╔═╡ 773db6a7-6f75-4a29-a49b-b7426e5b8d91
md"## caveats

* we assumed independent decisions among the customers. probably the drinks chosen by earlier customers influence the choices of later customers. for example, customers may see me drinking 🍷 then think \"the cool ppl are choosing🍷, I'll order that too.\" people might order in groups too---and influence each others' decisions.
...
* drink preferences might change based on time of day, weather, season, and day of the week.
